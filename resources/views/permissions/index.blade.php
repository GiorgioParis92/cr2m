...
@section('content')
    <div class="container">
        <h1>Permissions d'acces aux étapes</h1>
        ...
        <h1>Permissions de validation d'etapes</h1>
                        ...
                        <tr>
                            ...
                            @foreach ($chunk as $etape)
                                @foreach ($client_array as $key => $client)
                                    <td style="text-align:center">
                                        @if (
                                            (isset($permission_array['validate_'.$etape->etape_name][$type->id][$key]) &&
                                                $permission_array['validate_'.$etape->etape_name][$type->id][$key] == 1) ||
                                                !isset($permission_array['validate_'.$etape->etape_name][$type->id][$key]))
                                            <i data-name="{{ 'validate_'.$etape->etape_name }}" data-type_user="{{ $type->id }}"
                                                data-type_client="{{ $key }}" onclick="update_permission(this)"
                                                class="fa fa-circle-check text-success"></i>
                                        @endif
                                        @if (isset($permission_array['validate_'.$etape->etape_name][$type->id][$key]) &&
                                                $permission_array['validate_'.$etape->etape_name][$type->id][$key] == 0)
                                            <i data-name="{{ 'validate_'.$etape->etape_name }}" data-type_user="{{ $type->id }}"
                                                data-type_client="{{ $key }}" onclick="update_permission(this)"
                                                class="fa fa-circle-xmark text-danger"></i>
                                        @endif
