...
@foreach ($docs as $doc)
    ...
    @if ($doc['required'] == 1 || ($doc['required'] == 0 && isset($doc['meta_value']) && !empty($doc['meta_value'])))
        ...
        <span class="badge badge-md badge-circle badge-floating badge-danger border-white btn-outline-success text-danger" style="border: 1px solid white;" >{{$doc['order_column']}}</span> {!! $doc['title']. ' '.$text !!}
        </div>
    @endif
@endforeach
