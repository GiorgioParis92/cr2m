...
class Textarea extends AbstractFormData
{
    public function render(bool $is_error)
    {
        ...
        $data .= '<textarea wire:model="'.$wireModel.'" wire:keyup="update_value(\''.$wireModel.'\',  $event.target.value)" wire:blur="update_value(\''.$wireModel.'\',  $event.target.value)" class="form-control" name="'.($this->config->name ?? '').'">'.($this->value ?? "") .'</textarea>';
